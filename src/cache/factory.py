...
def get_cache_client(
    ...
    try:
        ...
        # Test connection - skip if event loop already running
        import asyncio
        try:
            # Check if event loop is already running (async context)
            try:
                asyncio.get_running_loop()
                # Event loop is running, skip ping test to avoid "already running" error
                logger.info("Event loop already running, skipping Redis ping (will test on first use)")
                return redis_client
            except RuntimeError:
                # No running loop, safe to test synchronously
                try:
