...
class DataValidatorAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        # Get API key from environment (Mistral AI)
        api_key = self.get_env("MISTRAL_API_KEY")

        # Create Pydantic AI agent with Mistral
        self.agent = Agent(
            "mistral:mistral-small-latest",  # Efficient model with good quota management
            output_type=DataValidatorResponse,  # Structured output type
